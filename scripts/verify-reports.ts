...
async function main() {
  ...
  try {
    ...
    // إيراد = ٢×٩٠ شحن = ١٨٠ ج (رسم التحصيل بيتحسب عند التسوية مش عند التسليم)
    check("   الإيراد ١٨٠ ج (شحن)", m?.revenueP, "18000");
    // مفيش عمولة متسجّلة → الربح = الإيراد
    check("   الربح = الإيراد (مفيش عمولة/تعويض)", m?.profitP, m?.revenueP);
    check("   العمولات ٠", m?.commissionP, "0");
